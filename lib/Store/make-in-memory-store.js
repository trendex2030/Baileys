...
ev.on('contacts.update', async (updates) => {
    ...
    for (const update of updates) {
        ...
        if (contacts[update.id]) {
            ...
            // Filter out null values from contactHashes
            const validContactHashes = contactHashes.filter(hash => hash !== null);

            contact = contacts[((_a = validContactHashes.find(([, b]) => b === update.id)) === null || _a === void 0 ? void 0 : _a[0]) || '']; // find contact by attrs.hash, when user is not saved as a contact
        }
